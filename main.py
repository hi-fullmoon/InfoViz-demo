...
def process_text_with_crewai(text: str) -> Dict[str, Any]:
    ...
    return {
        "information_processing_result": (
            str(information_processing_task.output)
            if information_processing_task.output
            else "未完成"
        ),
        "visualization_result": (
            extract_json_from_markdown(str(visualization_task.output))
            if visualization_task.output
            else "未完成"
        ),
...
def main():
    ...
    try:
        ...
        serializable_results = {
            "information_processing_result": str(
                results.get("information_processing_result", "")
            ),
